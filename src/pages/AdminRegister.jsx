      ...
      >
        <div className="auth-modal-header">
          <div>
            ...
            <h2 id="otp-modal-title">Enter the 6-digit code</h2>
          </div>
        ...
        <p className="auth-modal-copy">
          We sent a verification code to the email address on the registration form. Use the code below to complete admin account setup.
        </p>
...
export default function AdminRegister() {
  ...
  const handleSendCode = async (event) => {
    ...
    try {
      const response = await authService.registerSendCode(fullName, employeeCode, email, 'admin');
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setVerificationSuccess(response.message || 'Admin account verified successfully.');
      setOtpOpen(false);
    } catch (verifyError) {
  ...
  return (
    <AuthLayout
      ...
      subtitle="Register a new admin profile, receive a verification code, and complete setup in one flow."
    >
      <form className="register-form" onSubmit={handleSendCode} noValidate>
        ...
        {verificationSuccess && (
          <div className="alert alert-success" role="status" aria-live="polite">
            <span>{verificationSuccess} Redirecting to admin login...</span>
          </div>
        )}

        <div className="form-group">
          <label className="form-label" htmlFor="registerFullName">Name</label>
          <input
            id="registerFullName"
            type="text"
        ...
        <div className="form-group">
          <label className="form-label" htmlFor="registerEmployeeCode">Employee Code</label>
          <input
